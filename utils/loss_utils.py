...
def src2ref(ref_intrinsic, ref_extrinsic, ref_view_depth, dummy_intrinsic, dummy_extrinsic, dummy_view_depth, dummy_image):
    ...
    u, v = torch.meshgrid(torch.arange(ref_width, device=device), torch.arange(ref_height, device=device), indexing="xy")
    u, v = u.flatten().to(torch.float32) + 0.5, v.flatten().to(torch.float32) + 0.5

    z_ref = ref_view_depth.flatten()
    uv1 = torch.stack((u * z_ref, v * z_ref, z_ref), dim=0)
    xyz_ref = torch.matmul(torch.linalg.inv(ref_intrinsic), uv1)
    xyz_ref_homo = torch.cat((xyz_ref, torch.ones((1, xyz_ref.shape[1]), device=device)), dim=0)
    xyz_world = torch.matmul(torch.linalg.inv(ref_extrinsic), xyz_ref_homo)

    xyz_src = torch.matmul(dummy_extrinsic, xyz_world)[:3, :]
    uv_src = torch.matmul(dummy_intrinsic, xyz_src)
    ...
    grid = torch.stack((u_src, v_src), dim=-1).unsqueeze(0)
    reprojected_depth = torch.nn.functional.grid_sample(dummy_view_depth.unsqueeze(0).unsqueeze(0), grid, mode="bilinear", align_corners=True)
