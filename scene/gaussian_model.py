...
class GaussianModel:
    ...
    def load_blocks_ply(self, plys_dirpath):
        ...
        self._xyz = torch.tensor(xyz_all, dtype=torch.float, device="cuda")
        self._features_dc = torch.tensor(features_dc_all, dtype=torch.float, device="cuda").transpose(1, 2).contiguous()
        self._features_rest = torch.tensor(features_extra_all, dtype=torch.float, device="cuda").transpose(1, 2).contiguous()
        self._opacity = torch.tensor(opacities_all, dtype=torch.float, device="cuda")
        self._scaling = torch.tensor(scales_all, dtype=torch.float, device="cuda")
        self._rotation = torch.tensor(rots_all, dtype=torch.float, device="cuda")
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, block_bbx=None, N=2):
        ...
        if block_bbx is not None:
            ...
            block_bbx_mask = (xyz_block[:, 0] >= -x_extent) & (xyz_block[:, 0] <= x_extent) &                (xyz_block[:, 2] >= -z_extent) & (xyz_block[:, 2] <= z_extent) &                (xyz_block[:, 1] >= -y_extent*1.2) & (xyz_block[:, 1] <= y_extent*1.2)
